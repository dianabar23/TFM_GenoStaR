...
df_metilacion_t$Nivel_met_cg02084114 <- case_when(
  df_metilacion_t$decil_cg02084114 %in% 1:4 ~ "bajo",
  df_metilacion_t$decil_cg02084114 %in% 7:10 ~ "alto",
  TRUE ~ NA_character_
)

df_metilacion_t$Nivel_met_cg04706801 <- case_when(
  df_metilacion_t$decil_cg04706801 %in% 1:4 ~ "bajo",
  df_metilacion_t$decil_cg04706801 %in% 7:10 ~ "alto",
  TRUE ~ NA_character_
)

df_metilacion_t$Nivel_met_cg12694063 <- case_when(
  df_metilacion_t$decil_cg12694063 %in% 1:4 ~ "bajo",
  df_metilacion_t$decil_cg12694063 %in% 7:10 ~ "alto",
  TRUE ~ NA_character_
)

df_metilacion_t$Nivel_met_cg13596235 <- case_when(
  df_metilacion_t$decil_cg13596235 %in% 1:4 ~ "bajo",
  df_metilacion_t$decil_cg13596235 %in% 7:10 ~ "alto",
  TRUE ~ NA_character_
)
...
p1 <- ggplot(df_metilacion_t[!is.na(df_metilacion_t$Nivel_met_cg02084114), ],
             aes(x = Nivel_met_cg02084114, y = CYP3A5_exp, fill = Nivel_met_cg02084114))  +
  geom_boxplot(alpha = 0.7, color = "black") +
  scale_fill_manual(values = c("bajo" = "#56B4E9", "alto" = "#E69F00")) +
  labs(
    title = "cg02084114",
...
p2 <-ggplot(df_metilacion_t[!is.na(df_metilacion_t$Nivel_met_cg04706801), ],
            aes(x = Nivel_met_cg04706801, y = CYP3A5_exp, fill = Nivel_met_cg04706801))  +
  geom_boxplot(alpha = 0.7, color = "black") +
  scale_fill_manual(values = c("bajo" = "#56B4E9", "alto" = "#E69F00")) +
  labs(
    title = "cg04706801",
...
p3 <- ggplot(df_metilacion_t[!is.na(df_metilacion_t$Nivel_met_cg12694063), ],
             aes(x = Nivel_met_cg12694063, y = CYP3A5_exp, fill = Nivel_met_cg12694063)) +
  geom_boxplot(alpha = 0.7, color = "black") +
  scale_fill_manual(values = c("bajo" = "#56B4E9", "alto" = "#E69F00")) +
  labs(
    title = "cg12694063",
...
p4 <- ggplot(df_metilacion_t[!is.na(df_metilacion_t$Nivel_met_cg13596235), ],
             aes(x = Nivel_met_cg13596235, y = CYP3A5_exp, fill = Nivel_met_cg13596235)) +
  geom_boxplot(alpha = 0.7, color = "black") +
  scale_fill_manual(values = c("bajo" = "#56B4E9", "alto" = "#E69F00")) +
  labs(
    title = "cg13596235",
