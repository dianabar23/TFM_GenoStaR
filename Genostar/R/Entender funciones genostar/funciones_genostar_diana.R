...
#'
#' @export
find_missing_data_diana <- function(df) {
...
#' @export
fill_empty_cells_diana <- function(df) {
...
#' @export
process_cyp2d6_diana <- function(genotypes, row_idx, diplotypes, cnv_x9, cnv_int6, cnv_5utr, score, total_score) {
...
process_cyp2d6_adapatada <- function(genotypes, row_idx, diplotypes, cnv_x9, cnv_int6, cnv_5utr, score, total_score) {
  #initialize variables 
  allele_matches <- vector("character", 2)
  genotype_match <- FALSE
  seq_unmatched <- matrix(nrow = 0, ncol = 0)
...
#' @return A matrix containing any unmatched haplotypes, indicating further processing is required
#' @export
process_cyp1a2_diana <- function(genotypes, row_idx, diplotypes, score, total_score, CYP1A2_name){
  #initialize variables 
  allele_matches <- vector("character", 2)
  genotype_match <- FALSE
  seq_unmatched <- matrix(nrow = 0, ncol = 0)
...
#' @export
find_diplotype_diana <- function(df, genes, CYP1A2_name){
  ...
  for (gene in genes) {
    ...
    if (length(valid_snps_clean) > 0) {
      ...
      data_filled <- fill_empty_cells_diana(data_filtered)
      ...
      for (row_idx in 1:nrow(df_gene)) {
        ...
        if (gene == "CYP2D6"){
          ...
          cyp2d6_result <- process_cyp2d6_diana(genotypes, row_idx, diplotypes, cnv_x9, cnv_int6, cnv_5utr, score, total_score) 
        ...
        if(gene == "CYP1A2"){
          cyp1a2_result <- process_cyp1a2_diana(genotypes, row_idx, diplotypes, score, total_score, CYP1A2_name)
...
#' @export
star_to_activity_diana <- function(df, gene) { 
  gene <- toupper(gene)
  data <- switch(gene,
                 "CYP2C9" = CYP2C9_data,
...
#' @export
star_to_pheno_diana <- function(df, gene) {
  gene <- toupper(gene)
...
#' @export
generate_phased_combinations_diana <- function(snp_df, snp_cols) {
...
#' @export
format_genotype_diana <- function(genotype) {
  if (length(genotype) > 1) {
    stop("Unexpected vector input in format_genotype_diana: ", paste(genotype, collapse = ", "))
  }
...
#' 
#' @export
adjust_diplotype_diana <- function(final_results) {
    ...
    for (i in seq_len(nrow(final_results))) {
      if (final_results$CYP2D6_diplotype[i] %in% c("*10/*4", "*4/*10")) {
        ...
        if ((rs1065852 %in% c("A G", "G A", "AG", "GA")) & (rs3892097 %in% c("C T", "T C", "CT", "TC"))) {
          ...
          rs1065852 %in% c("AA", "A A") &
          rs3892097 %in% c("CT", "TC", "C T", "T C")
        ) {
          final_results$CYP2D6_diplotype[i] <- "*4/*10"
          
        } else if (
          rs1065852 %in% c("AA", "A A") &
          rs3892097 %in% c("TT", "T T")
        ) {
...
#' @export
reorder_diplotype_diana <- function(diplotype) {
